...
function make_string_row(sheet, r, R, cols, fs, rs, FS, o) {
	var row = [];
	var isempty = true;
	var txt = "", rr = encode_row(R);
	for(var C = r.s.c, CLen = r.e.c; C <= CLen; ++C) {
		var val = o.dense ? (sheet[R]||[])[C]: sheet[cols[C] + rr];
		if(val == null) txt = "";
		else if(val.v != null) {
			isempty = false;
			txt = '' + val.w;
			// no need for any special formating
			//txt = '' + format_cell(val, null, o);
			//for(var i = 0, cc = 0; i !== txt.length; ++i) if((cc = txt.charCodeAt(i)) === fs || cc === rs || cc === 34) {
				//txt = "\"" + txt.replace(qreg, '""') + "\""; break; }
		} else if(val.f != null && !val.F) {
...
function sheet_to_multiArray(sheet, opts) {
	var out = [];
	// var out = '';
	var o = opts == null ? {} : opts;
	if(sheet == null || sheet["!ref"] == null) return "";
	...
	var row = [], cols = [];
	o.dense = Array.isArray(sheet);
	for(var C = r.s.c, CLen = r.e.c; C <= CLen; ++C) cols[C] = encode_col(C);
	for(var R = r.s.r, RLen = r.e.r; R <= RLen; ++R) {
		row = make_string_row(sheet, r, R, cols, fs, rs, FS, o);
		if(row == null) { continue; }
		// commented row is now array than string
		//if(o.strip) row = row.replace(endregex,"");
		//out += row + RS;
		out.push(row);
	}
...
var utils = {
	...
	sheet_to_multiArray: sheet_to_multiArray
};
